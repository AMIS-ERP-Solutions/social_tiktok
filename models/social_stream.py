    ...
    def _fetch_tiktok_user_videos(self):
        ...
        try:
            response = requests.post(
                url_join(self.env['social.media']._TIKTOK_API_ENDPOINT, 'video/list/'),
            ...
            _logger.warning("Social TikTok: Network error fetching videos for '%s': %s", account.name, e)
            return False

        if not response.ok:
            _logger.warning("Social TikTok: Failed to fetch video list for '%s': %s", account.name, response.text)
